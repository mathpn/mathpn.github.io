...
Prepend is -- by far -- the fastest function. This comes down to the trade-offs of singly-linked lists. Iterating is quite simple, you get the head value and follow the tail pointer to the next head, so it's quite fast. To prepend we only need to create a new head, find the current head and point the new head to the current head, which is now a tail. This can be done in constant time O(1) since the current head location in memory is always known.

However, random access is also slow since each access requires iterating over the list up to the desired index. There is no way to directly access one position since the list is not represented as a contiguous memory space, therefore we don't know where the nth value is stored in memory. Each random access operation happens in linear time -- O(n). Similarly, to append an element to the end of the list we need to find the last element. We must iterate over the list following the tail pointers until we reach its end. Thus, this is also done in linear time -- O(n).
