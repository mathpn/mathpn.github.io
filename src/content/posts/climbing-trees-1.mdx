...
$$
\text{Misclassification Rate} = \frac{\sum_{i=1}^N \mathbb{I}(\hat{y}_i \neq y_i)}{N}
$$

This is a very intuitive objective for classification. It measures the proportion of misclassified examples. The prediction $\hat{y}$ is the majority vote of the node. Our goal is to decrease leaf node impurity, which is aligned with the misclassification rate function. However, this is not a good objective and is not used during training because it is not sensitive enough.

### Gini impurity

The Gini impurity[^gini_confusion] over a set of class probabilities $p$ with $c$ classes is defined as:

[^gini_confusion]: Not to be confused with the [Gini coefficient](https://en.wikipedia.org/wiki/Gini_coefficient)

$$
I_{G}(p) = \sum_{k=1}^{c} p_{k}(1 - p_{k}) = 1 - \sum_{k=1}^c p_{k}^2
$$
...
The worst Gini impurity arises when the probabilities are uniform among classes, that is, a node with maximum impurity is no better than a coin toss at classifying examples.
It can be shown that the upper bound of Gini impurity is 1 as the number of classes grow.

### Entropy

$$
H(p) = - \sum_{k=1}^{c} p_{k}\ log_{2} p_{k}
$$
