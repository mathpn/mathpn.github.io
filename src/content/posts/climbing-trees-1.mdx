...
In all examples, consider the data $S = \{(x_1, y_1), ..., (x_n, y_n)\}, y_i \in \{1, ..., c\}$ where $c$ is the number of classes.

### Misclassification rate

$$
MR = \frac{\sum_{i=1}^n \mathbb{I}(\hat{y}_i \neq y_i)}{n}
$$
...
$$
G(S) = \sum_{k=1}^{c} p_{k}(1 - p_{k}) = 1 - \sum_{k=1}^c p_{k}^2
$$
...
Where $\frac{|S_L|}{|S|}$ is the fraction of inputs in the left child node and $\frac{|S_R|}{|S|}$, in the right.
...
We can come up with splits that have the same misclassification rate, but one has a pure node while the other doesn't. For instance:
...
$$
MR(S) = \frac{10}{25} 0 + \frac{15}{25} \frac{5}{15} = 0.2
$$

$$
MR(S') = \frac{12}{25} \frac{2}{12} + \frac{13}{25} \frac{3}{13} = 0.2
$$
...
$$
G(S) = \frac{10}{25} 0 + \frac{15}{25} 0.444 = 0.266
$$

$$
G(S') = \frac{12}{25} 0.277 + \frac{13}{25} 0.355 = 0.318
$$
...
More formally, both Gini impurity and entropy are strictly concave, while misclassification rate is not.
